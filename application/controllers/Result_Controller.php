	...
	class Result_Controller extends CI_Controller {
		...
		/*	public function my_DOMPDF(){
			...
			//	if($new_semester == 1 && $level == 1) {$new_level = 1;}
				...
				if($level == 1 && $semester == 1) {$num_of_rows_for_prev_sem_in_marks_table = 1;}
				...
				while(1) {
					while(1) {
					   ...
					   $get_cch['value'] = $this->Result_Model->fetch_cch_from_syllabus_table($c_code_for_gpa);
						...
						if($f == 1) break;
						if($temp_level == 1 && $temp_semester == 1) { $f = 1; break;}
						if($temp_semester == 2) $temp_semester = 1;
						else if($temp_semester == 1) {
							$temp_semester = 2; $temp_level -= 1; 
						}
					}
					...
					if($change_semester == 1)  {
						$temp_semester = 2; $temp_level = $update_level; $change_semester = 2;
					}
					else if($change_semester == 2) {
						$temp_semester = 1; $temp_level = $change_level + $k; 
						$update_level = $change_level + $k;
						$change_semester = 1; $k += 1;
					}
				...
				public function pdfdetails() {
				 ...
				 //  $html .= '<img src = "C:\xampp\htdocs\CodeIgniter\images\logo_transparent.png" alt = "hello" height = "80" width = "82">';
				  ...
				  // $html .= '';
				    ...
				    $html .= '<h2><br><br>PATUAKHALI SCIENCE AND TECHNOLOGY UNIVERSITY</h2>';
				    $html .= '</th>';
				    $html .= '</tr>';
				    $html .= '</table>';
					$html .= '<h3>Dumki, Patuakhali-8602, Bangladesh</h1>';
					$html .= '<h1>Examination Result Tabulation</h1>';
					$html .= '</div>';
				    $html .= '</td>';
